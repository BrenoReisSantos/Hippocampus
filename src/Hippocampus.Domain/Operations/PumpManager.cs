...
public class PumpManager(WaterTank _waterTank)
{
    ...
    {
        if (IsBypassingPumpRules()) return _waterTank with { PumpingWater = true };
        if (CanPump()) return _waterTank with { PumpingWater = true };
        return _waterTank;
    }
    ...
    {
        if (_waterTank.PumpsTo is null) return false;
